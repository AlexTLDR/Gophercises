...
//  first exercise (basicMapExercises.go), and try them by looking up for the keys.
//
...
func main() {
	...
	//Print Dulin's phone no:
	key, phone := "dulin", "-"
	if v, ok := phones[key]; ok {
		phone = v
	}
	...
	//What is Greco's second phone number?
	key, phone = "greco", "-"
	if phones := multiPhones[key]; len(phones) >= 2 {
		phone = phones[1]
	}
